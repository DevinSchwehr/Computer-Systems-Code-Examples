...
void eval(char *cmdline) 
{
  ...
  else
  {
    ...
    {
      int pid;
      //Block out the SIGCHLD signals with a mask
      sigset_t set, oset;
      sigemptyset(&set);
      ...
      //Now that we've added the child to the joblist, unblock those signals.
      sigprocmask(SIG_UNBLOCK, &set, &oset);
      struct job_t *curr_job;
      curr_job = getjobpid(jobs,pid);
      if(pid == 0)
      {
        //Put the child in a new group and execute
        setpgid(0,0);
      ...
      //Or print out that it's a background job
      else
      {
        printf("[%d] (%d) %s",curr_job->jid,curr_job->pid,cmdline);
      }
    }
  }
  return;
...
int builtin_cmd(char **argv) 
{
  ...
  //Following two commands are ones that I made, for the quit and jobs commands respectively.
  if (!strcmp(cmd, "quit"))
  {
    exit(0);
  }
...
void do_bg(int jid) 
{
  ...
  //Make sure that the job actually exists
  if(curr_job == NULL)
  {
    sio_puts("%");
    sio_putl(jid);
  ...
  //If it does, send SIGCONT and print out relevant information.
  curr_job->state = BG;
  kill(-curr_job->pid, SIGCONT);
  sio_puts("Job [");
  sio_putl(curr_job->jid);
  sio_puts("] (");
  sio_putl(curr_job->pid);
  sio_puts(") ");
  sio_puts(curr_job->cmdline);
...
void do_fg(int jid) 
{
  ...
  //Set it's state to FG and send it the signal to continue
  curr_job->state = FG;
  kill(-curr_job->pid, SIGCONT);
  waitfg(curr_job->pid);
  return;
...
void waitfg(pid_t pid)
{
  ...
  while(1)
  {
    //Wait until the current job is either no longer the foreground job or completed.
    sleep(1);
    if(curr_job->state != FG || curr_job == NULL)
    {
      break;
    }
  }
...
void sigchld_handler(int sig) 
{
  while(1)
  {
    ...
    //The three if statements below decide whether it exited normally, received a SIGINT signal or was stopped
    //by SIGTSTP
    if(WIFEXITED(status))
    {
      deletejob(jobs,pid);
